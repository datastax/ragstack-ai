...
RUN cd libs/langflow && $POETRY_HOME/bin/poetry run pip show langflow | grep Location | cut -d ' ' -f 2 | xargs -I {} cp -r {}/langflow/frontend /tmp/frontend
...
COPY --chown=nginx ./docker/langflow/frontend/nginx.conf /etc/nginx/conf.d/default.conf
COPY --chown=nginx ./docker/langflow/frontend/start-nginx.sh /start-nginx.sh
