...
class Node:
    ...
    def incoming_links(self) -> set[Link]:
        set([l for l in self.links if (l.direction in ["in", "bidir"])])

    def outgoing_links(self) -> set[Link]:
        set([l for l in self.links if (l.direction in ["out", "bidir"])])
...
def _row_to_node(row: Any) -> Node:
    if "metadata_blob" in row:
        metadata = _deserialize_metadata(row["metadata_blob"])
        links: list[Link] = metadata.get("links", [])
    else:
        metadata = {}
        links = []
    return Node(
        id=row[CONTENT_ID] if CONTENT_ID in row else "",
        embedding=row["text_embedding"] if "text_embedding" in row else [],
        text=row["text_content"] if "text_content" in row else "",
        metadata=metadata,
        links=set(links),
    )
...
class GraphStore:
        ...
        *,
        ...
        with self._concurrent_queries() as cq:
            ...
            def visit_targets(d: int, rows: Sequence[Any]) -> None:
                ...
                new_nodes_at_next_depth: Set[Node] = set()
                for row in rows:
                    target_node = _row_to_node(row=row)
                    if d < visited_ids.get(target_node.id, depth):
                        new_nodes_at_next_depth.add(target_node)

                for node in new_nodes_at_next_depth:
                    cq.execute(
            ...
            cq.execute(
                ...
                callback=lambda rows: visit_nodes(0, rows=rows),
            )
    ...
    def _get_search_cql(
        ...
        if len(metadata_keys) == 0 and select_cql in self._prepared_query_cache:
    ...
    def _get_search_cql_and_params(
        ...
        metadata: dict[str, Any] | None = None,
        embedding: list[float] | None = None,
        search_links: list[Link] | None = None,
    ) -> tuple[PreparedStatement, tuple[Any, ...]]:
        if search_links is not None:
            ...
            for search_link in search_links:
                metadata[_metadata_s_link_key(search_link)] = _metadata_s_link_value()
