...
class CassandraDatabase(BaseDatabase):
    ...
    def _put_async(self, doc_id: str, chunk_id: int, embedding_id: Optional[int] = None, text: Optional[str] = None, metadata: Optional[Dict[str,Any]] = None, vector: Optional[Vector] = None) -> ResponseFuture:
        if vector is None:
            return self._table.put_async(
                partition_id=doc_id,
                row_id=(chunk_id, -1),
                body_blob=text,
                metadata=metadata,
    ...
    def add_chunks(self, chunks: List[Chunk]) -> List[Tuple[str, int]]:
        ...
        def _init_put(doc_id: str, chunk_id: int, embedding_id: int, text: Optional[str] = None, metadata: Optional[Dict[str,Any]] = None, vector: Optional[Vector] = None):
            future = self._put_async(doc_id=doc_id, chunk_id=chunk_id, embedding_id=embedding_id, text=text, metadata=metadata, vector=vector)
        ...
        for chunk in chunks:
            ...
            _init_put(doc_id=doc_id, chunk_id=chunk_id, embedding_id=-1, text=text, metadata=metadata)

            for index, vector in enumerate(chunk.embedding):
                _init_put(doc_id=doc_id, chunk_id=chunk_id, embedding_id=index, vector=vector)
    ...
    async def aadd_chunks(self, chunks: List[Chunk]) -> List[Tuple[str, int]]:
        ...
        def _init_put(doc_id: str, chunk_id: int, embedding_id: int, text: Optional[str] = None, metadata: Optional[Dict[str,Any]] = None, vector: Optional[Vector] = None):
            ...
            future.add_callbacks(
                callback=_result_callback, callback_args=(task_id,),
                errback=_error_callback, errback_args=(task_id,),
            )
    ...
    async def adelete_chunks(self, doc_ids: List[str]) -> bool:
        ...
        for doc_id in doc_ids:
            ...
            future.add_callbacks(
                callback=_result_callback, callback_args=(doc_id,),
                errback=_error_callback, errback_args=(doc_id,),
            )
