...
from ragstack_langchain.graph_store.extractors import HtmlLinkExtractor, LinkExtractorTransformer
from ragstack_langchain.graph_store.extractors.gliner_link_extractor import GLiNERLinkExtractor
from ragstack_langchain.graph_store.extractors.keybert_link_extractor import KeybertLinkExtractor
from ragstack_langchain.graph_store.links import get_links, Link
from . import test_html_link_extractor, test_gliner_link_extractor, test_keybert_link_extractor
...
def test_multiple_extractors():
    transformer = LinkExtractorTransformer([
        ...
        GLiNERLinkExtractor(labels=["Person", "Award", "Date", "Competitions", "Teams"]),
    ])
    ...
    assert get_links(doc2) == {
        ...
        Link(kind='entity:Competitions',
             direction='bidir',
             tag='UEFA Champions Leagues'),
        Link(kind='entity:Competitions',
             direction='bidir',
             tag='UEFA\nNations League'),
        Link(kind='entity:Award',
             direction='bidir',
