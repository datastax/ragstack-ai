...
def run_suite(
    ...
    for embedding_model in embedding_models:
        ...
        for threads in threads_per_benchmark:
            ...
            command = f"{sys.executable} -m pyperf command --copy-env -p {processes} -n {values_per_benchmark} -l {loops} --min-time 0.001 -o {abs_filename} -- {sys.executable} {benchmarks_dir}/testcases.py {logs_file} {test_name} {embedding_name} {batch_size} {chunk_size} {threads} {vector_database} {collection_name}"
            print(
