...
def test_csv_loader():
    ...
    with tempfile.NamedTemporaryFile(mode='w+', suffix='.csv') as temp_csv_file:
        with open(temp_csv_file.name, 'w') as write:
            write.writelines(["column1,column2,column3\n", "value1,value2,value3\n", "value4,value5,value6\n",
...
def test_web_based_loader():
    ...
    loader = WebBaseLoader(["https://langstream.ai/changelog/", "https://langstream.ai/faq/"])
    ...
    assert doc1.metadata == {'source': 'https://langstream.ai/changelog/',
                             'title': 'LangStream Changelog | LangStream: an Event-Driven Developer Platform for LLM Apps',
                             'description': 'Notes from releases', 'language': 'en'}

    doc2 = docs[1]
    assert "LangStream is an open-source project" in doc2.page_content
    assert doc2.metadata == {'source': 'https://langstream.ai/faq/',
                             'title': 'FAQ | LangStream: an Event-Driven Developer Platform for LLM Apps',
                             'description': 'Frequently Asked Questions', 'language': 'en'}
