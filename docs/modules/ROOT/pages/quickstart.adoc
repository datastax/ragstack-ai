...
This quickstart demonstrates a basic RAG pattern using RAGStack and the {db-vector} database to retrieve context and pass it to a language model for generation.
...
+
You must have an existing collection in {db-vector} (e.g. `test`). If you don't have one, create one in the {astra-ui}, or with the Data API (using the `.env` values from the next step):
+
[source,curl]
----
curl -v -s --location \
