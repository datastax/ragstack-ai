...
This doc describes two different advanced RAG techniques: MultiQueryRAG and ParentDocumentRAG.
...
For the purpose of this example, you will use a method that is compatible with the advanged ParentDocumentRAG technique.
...
======
+
. We see some of the questions were answered well, and others were not.
...
Now try the advanced MultiQueryRAG technique.
...
Remember that this will perform a post-processing step between retrieval and inference to replace the child documents with their parents. After this is done, any duplicate documents are removed.
...
. Run the ParentDocumentRRAG chain over the questions:
+
[tabs]
======
Python::
...
[source,python]
----
# Delete the collection - if this call returns a timeout, wait a few seconds and try again
