...
image::https://colab.research.google.com/assets/colab-badge.svg[align="left",link="https://colab.research.google.com/github/datastax/ragstack-ai-examples/blob/main/langchain_evaluation.ipynb"]
...
[source,python]
----
embedding = OpenAIEmbeddings(openai_api_key=oopenai_api_key)
vstore = AstraDB(
...
LangChain offers several built-in https://docs.smith.langchain.com/evaluation/evaluator-implementations[evaluators] to test the efficacy of your RAG pipeline. Since you've now created a RAG pipeline, the QA Evaluator is a good fit.
...
For more on evaluators, see https://docs.smith.langchain.com/evaluation/evaluator-implementations[LangChain evaluators]{external-link-icon}.
...
Find more examples xref:index.adoc[here].
