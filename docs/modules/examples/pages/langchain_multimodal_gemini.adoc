...
This notebook demonstrates using LangChain, Astra DB and a Google Gemini Pro Vision model to perform multi-modal Retrieval Augmented Generation (RAG).

== Prerequisites

You will need an {db-vector} database and a Google Cloud Platform account.
...
. Ask the LLM the same question, but this time you'll perform a vector search against Astra DB using the same image to supply the LLM with relevant products in the prompt.
+
[tabs]
======
Python::
